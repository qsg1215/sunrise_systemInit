...
class CongesMileSum extends Component {
  ...
  componentDidMount() {
    let chart = window.echarts.init(document.getElementById('areaLine'), this.props.theme);
    ...
    this.themeChangeSubscribe = PubSub.subscribe('themeChange', (type, theme) => {
      chart.dispose();
      chart = window.echarts.init(document.getElementById('areaLine'), theme);
      chart.clear();
      chart.setOption(option);
    });
  }
  ...
  render() {
    return (
      <div id="areaLine" className={styles.areaLine}>
        123
      </div>
    );
  }
